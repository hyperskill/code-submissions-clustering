...
from src.utils.file_utils import create_dir, list_dirs, get_absolute_path
...
def build_initial_graph_filename(step_id: int, args: argparse.Namespace) -> str:
    ...
    """
    input_path = get_absolute_path(args.input_path)
...
if __name__ == '__main__':
    ...
    create_dir(get_absolute_path(args.output_path))
    logger = set_logger(get_absolute_path(args.output_path))

    step_ids = [int(d) for d in list_dirs(get_absolute_path(args.input_path))]
